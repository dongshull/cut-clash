...
export default {
  ...
  methods: {
    ...
    async getSubUrl() {
      try {
        ...
        if (!regexCheck(this.api)) {
          this.showErrorDialog('错误的后端API地址');
          return;
        }
        const response = await getSubLink(this.api, {
          urls: this.urls,
          target: this.target,
          moreConfig: this.moreConfig,
          remoteConfig: this.remoteConfig,
        });
        this.result.subUrl = response;
        this.$forceUpdate();
      } catch (e) {
        showNotification({
          type: 'error',
    ...
    async getShortUrl() {
      try {
        showLoading('生成中...');
        const response = await request(`${this.shortUrl}/api`, 'post', {
          url: this.result.subUrl,
        });
        if (response.code == 200) {
          this.result.shortUrl = response.data;
        } else {
          this.showErrorDialog(response.message || '出错了');
        }
      } catch (e) {
